...
public class ReflectiveAspectJAdvisorFactory extends AbstractAspectJAdvisorFactory implements Serializable {
	...
	public List<Advisor> getAdvisors(MetadataAwareAspectInstanceFactory aspectInstanceFactory) {
		...
		// DeclareParents 主要是用于引介增强的注解形式的实现，而其实现方法与普通增强很类似，
	...
	public Advisor getAdvisor(Method candidateAdviceMethod, MetadataAwareAspectInstanceFactory aspectInstanceFactory,
		...
		/**
		 * 在封装过程中只是简单地将信息封装在类的实例中，所有的信息单纯地赋值，在实例初始化的过程中还完成了对于
		 * 增强器的初始化，因为不同的增强所体现的逻辑是不同的，比如 @Before("test()") 与 @After("test()") 标签的不同
	...
	private AspectJExpressionPointcut getPointcut(Method candidateAdviceMethod, Class<?> candidateAspectClass) {
		...
		// 提取得到的注解中的表达式，如：
		// @Pointcut 中的 execution(* com.zhenghao.spring.context.service.impl.TestServiceImpl.*(..))
	...
	protected static class SyntheticInstantiationAdvisor extends DefaultPointcutAdvisor {
		...
		public SyntheticInstantiationAdvisor(final MetadataAwareAspectInstanceFactory aif) {
			// 目标方法前调用，类似 @Before
			// aif.getAspectInstance() 简单初始化 aspect
