...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 进入 prepareBeanFactory 函数前，spring 已经完成了对配置文件的解析，而 ApplicationContext 在功能上的扩展也由此展开
			...
			try {
				...
				// 激活各种 BeanFactory 后处理器
				// 1) 扫描 各种 bean 扔到 beanDefinition
				// 2) 处理各种 @import
				// 3) 如果有扩展 BeanFactoryPostProcessor ，则将执行 BeanFactoryPostProcessor 接口中的方法
				// 扫描scan --> put beanDefinitionMap --> 执行 BeanFactoryPostProcessor(1：自定义的。2：内置的)
				...
				// 初始化应用消息广播器(推送事件给所有监听器的)，并放入 “applicationEventMulticaster” bean 中
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 设置 beanFactory 的 classLoader 为当前的 classLoader
		beanFactory.setBeanClassLoader(getClassLoader());
		// 设置 beanFactory 的表达式语言处理器， Spring3 增加了表达式语言的支持
		// 默认可以使用 #{bean.xxx} 的形式来调用相关属性
		beanFactory.setBeanExpressionResolver(new StandardBeanExpressionResolver(beanFactory.getBeanClassLoader()));
		// 为 beanFactory 增加一个默认的 propertyEditor ，主要是对 bean 的属性等设置管理的一个工具
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		 ...
		 * 如果加了 getBeanFactoryPostProcessors() 这个地方得不得，是spring 自己扫描的
	...
	protected void initMessageSource() {
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			// 如果在配置文件中已经配置了 messageSource ，那么将 messageSource 提取并记录在 this.messageSource 中
	...
	protected void registerListeners() {
		...
		// 这里不初始化 FactoryBeans :我们需要保留所有常规bean未初始化，让后处理器应用于他们!
		// 配置文件注册的监听器处理
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// Instantiate all remaining (non-lazy-init) singletons.
		// 初始化剩下的单实例（非惰性的），上面都在实例化 Spring 的 bean,就在这里，实例化我们自己的 bean
	...
	protected void finishRefresh() {
		...
		// 当 ApplicationContext 启动或停止时，它会通过 LifecycleProcessor 来与所有声明的 bean 的
